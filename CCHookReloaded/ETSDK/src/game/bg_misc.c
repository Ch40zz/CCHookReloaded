...
weapon_t BG_WeaponForMOD( int MOD ) {
	...
	for(i = 0; i < (weapon_t) WP_NUM_WEAPONS; i++) {
		if(GetAmmoTableData((weapon_t) i)->mod == MOD) {
			return (weapon_t) i;
		}
	}
...
int BG_colorstrncpyz(char *in, char *out, int str_max, int out_max)
{
	...
	const int in_len = (int) strlen(in);
...
volatile colorTable_t OSP_Colortable[] =
	{
		{ "white",		&colorWhite },
		{ "red",		&colorRed },
		{ "green",		&colorGreen },
